...
def feedback(request):
    # Get all contact submissions ordered by created_at in descending order (newest first)
    feedbacks = Contact.objects.all().order_by('-created_at')
    return render(request, "App/Feedbacks.html", {'feedbacks': feedbacks})
        ...
        try:
            ...
            if existing_search:
                if existing_search.combined_summary:
                    ...
                    guide_content = guide_result.get(
                        'guide') if guide_result.get('success') else None
                    audio_url = guide_result.get(
                        'audio_url', '') if guide_result.get('success') else ''
            ...
            def generate_content_when_ready():
                try:
                    ...
                    summary_result = generate_and_store_summaries(
                        search_query.id)
                    if summary_result['success']:
                        ...
                        if guide_result.get('success'):
                            search_query.guide_content = guide_result.get(
                                'guide')
                            search_query.save()
                        else:
                            print(
                                f"Guide generation failed: {guide_result.get('error')}")
                    else:
                        print(
            ...
            try:
                if os.path.exists(audio_dir):
                    audio_files = [f for f in os.listdir(
                        audio_dir) if f.startswith(f"guide_{search_query_id}_")]
                    if audio_files:
                        audio_path = os.path.join(
                            'audio_guides', audio_files[0])
                        audio_url = f'/media/{audio_path}'

                        full_path = os.path.join(
                            settings.MEDIA_ROOT, audio_path)
...
def download_guide(request):
    ...
    try:
        ...
        response = HttpResponse(search_query.guide_pdf,
...
def download_pdf(request, query_id):
    ...
    if query.guide_pdf and query.pdf_filename:
        response = HttpResponse(
            query.guide_pdf, content_type='application/pdf')
